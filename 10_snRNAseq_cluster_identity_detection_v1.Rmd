...
start_data <- "QCd_norm_anchored_clustered_wscale_ws_MG_subset_20200910.rdata"
  ...
  if(!identical(cell_type, "organoid")){
    if(identical(cell_type, "pu1")){
      ...
      plot2<-FeaturePlot(ss_data_norm, 
            reduction = "umap", 
            features = c("C3", "CD74", "C1QB", "CX3CR1", "SPI1"))
      ggsave(filename = paste0(umaps_path,"/", cell_type, "_mglia_gene_expression_umap_", chose_resolution, suffix, ".png"), height = 7, width = 11, plot = plot2)
    
      #Plot genes by cluster for broad cell types
      feature_genes <- c("SPI1", "C3", "CD74", "C1QB", "CX3CR1","GFAP", "S100B", "ALDH1L1", "AQP4", "MAP2", "SATB2", "CALB2", "RBFOX3","SLC17A7", "GRIA1", "GRIN1", "GRIN2B","GAD1", "GAD2", "SLC6A1", "SLC6A11","NES", "SOX2", "DCX", "NEUROD1","PDGFRA", "COL9A1", "COL20A1", "SOX8", "MYT1", "TMEM63A", "ST18", "TGFBR2", "LEF1", "ITIH5", "CCR2")
      ...
      plot3 <- DoHeatmap(ss_data_norm, 
                        ...
                        hjust = 1,
                        angle = 0)
      ggsave(filename = paste0(umaps_path,"/", cell_type, "_general_gene_expression_cluster_heatmap_", chose_resolution, suffix, ".pdf"), height = 7, width = 11, plot = plot3)
      
      plot4 <- DotPlot(ss_data_norm, 
                        features = feat_in_data,
                        assay = assay) + RotatedAxis()
      ggsave(filename = paste0(umaps_path,"/", cell_type, "_general_gene_expression_cluster_dotplot_", chose_resolution, suffix, ".pdf"), height = 7, width = 11, plot = plot4)
      ...
      plot1<- FeaturePlot(ss_data_norm, 
       ...
       #Plot genes by cluster for broad cell types
      feature_genes <- c("SPI1", "C3", "CD74", "C1QB", "CX3CR1","GFAP", "S100B", "ALDH1L1", "AQP4", "MAP2", "SATB2", "CALB2", "RBFOX3","SLC17A7", "GRIA1", "GRIN1", "GRIN2B","GAD1", "GAD2", "SLC6A1", "SLC6A11","NES", "SOX2", "DCX", "NEUROD1","PDGFRA", "COL9A1", "COL20A1", "SOX8", "MYT1", "TMEM63A", "ST18", "TGFBR2", "LEF1", "ITIH5")
