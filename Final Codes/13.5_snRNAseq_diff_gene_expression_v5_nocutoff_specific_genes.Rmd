---
title: "13_snRNAseq_diff_gene_expression_v5_clust_no_cutoff"
...
change_output_path <- "../output/output_post-sub_no_ref/DEG/cluster_to_cluster/specific_genes/"
...
overwrite <- FALSE
    ...
    # leave as NULL
    write_name <- "Colin_etal_Senescence_"

# Set which assay will be analyzed. Seurat recommends using the normalized data
# from the "RNA" assay.
DEG_assay <- "RNA"

# Set the specific genes that you want to compare between clusters. Provide the # csv filenames without the .csv on the end. These must be located in the data
# folder.
specific_features <- c("Colin_etal_2018_senescence_geneset")
...
# Choose which DEG tests to run_-----------------------------------------------
  ...
  clust_to_specific_clust <- FALSE
    ...
    base_clust <- c(4,6)
    comp_clust <- c(1:10) #c(3, 4, 5, 6, 7, 9, 10)
...
if (is.null(specific_features)) {
  print("No specific genes to load. Moving on.")
} else {
    print(paste0("Loading specific gene list", specific_features))
    specific_genelist <- read_csv(paste0("../data/", specific_features, ".csv"))
    specific_genelist <- unlist(specific_genelist)
    specific_genelist <- toupper(specific_genelist)
    #specific_genelist <- intersect(unlist(ss_data_norm@raw.data@Dimnames[1]),
    #                               specific_genelist)
}
...
if (clust_to_specific_clust) {
  ...
  # Check whether genes from the specific_gene list are in the variable features
  feat_in_data <- list_features(specific_genelist)
