...
change_csv <- "pu1_APOE33_metadata.csv"
...
meta_vars <- c("Study_Designation", "Sex", "Age") #, "SeqBatch")
...
interactions <- c("Study_Designation:Age") #, "Study_Designation:SeqBatch")
...
if (!is.null(change_resolution)) {
  ...
  res_name <- paste0("no.ref_full22_integrated_snn_res.", chose_resolution)
...
bulk_path <- change_path(paste0(outpath, "/DEG/pseudobulk_by_group_mixed_clusters/"))
full_path <- change_path(paste0(outpath, "/DEG/pseudobulk_by_group_mixed_clusters/no_cutoff/"))
...
for (cluster in clusters) {
  ...
  bulk_path <- paste0(outpath, "/DEG/pseudobulk_by_group_mixed_clusters/")
  full_path <- paste0(outpath, "/DEG/pseudobulk_by_group_mixed_clusters/no_cutoff/")
  ...
  for (effect in names) {
    ...
    if (identical(method, "lfc") & dim(filter(results, padj < 0.1))[1] > 0) {
      ...
      for (pathway in pathway_list) {
        # Change output folders
        bulk_path <- change_path(paste0(outpath, "/DEG/pseudobulk_by_group_mixed_clusters/no_cutoff/Wald/"))
        plots_path <- change_path(paste0(bulk_path, "plots/"))
        ...
        if (dim(gsea_results@result)[1] > 0) {
          bulk_path <- change_path(paste0(outpath,
                                          "/DEG/pseudobulk_by_group_mixed_clusters/Wald/"))
          plots_path <- change_path(paste0(bulk_path, "plots/"))
