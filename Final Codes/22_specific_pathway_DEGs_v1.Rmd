---
title: "22_specific_pathway_DEGs_v1"
...
# Set the csv file containing the pathways you wish to investigate further.
# Please put the csv in the data folder of the project
pathway_csv <- "Pathways_of_interest.csv"
...
DEG_path <- change_path(paste0(outpath,
                               "/DEG/pseudobulk_by_group/paths_of_interest/"))
```

``` {r DESeq2_DEG}
# Read in pathway databases
...
# Set up cluster specific pathway list
pathway_list <- c("cluster_kegg", "cluster_react", "cluster_wp")
...
for (cluster in clusters) {
  ...
  bulk_path <- change_path(paste0(DEG_path, "cluster_", cluster, "/"))
  
  # Set the path to plot folders
  ...
  # Read in csv
  paths_of_interest <- readLines(paste0("../data/", pathway_csv))
  
  # Remove genelists that are not of interest
  all_paths <- pathways_kegg[pathways_kegg$term %in% paths_of_interest,]
  all_paths <- rbind(all_paths,
                     pathways_react[pathways_react$term %in% paths_of_interest,])
  all_paths <- rbind(all_paths,
                     pathways_wp[pathways_wp$term %in% paths_of_interest,])
  all_paths$term <- factor(all_paths$term, paths_of_interest)
  all_paths_list <- split(all_paths, all_paths$term)
  ...
  for (list in 1:length(all_paths_list)) {
    # Subset the counts to only the cluster and genes in the pathway of interest
    ...
    cluster_counts <- cluster_counts[which(rownames(cluster_counts) %in%
                                                  all_paths_list[[list]]$gene),]
    
    # Remove genes with zero counts in this cluster
    cluster_counts <- cluster_counts[rowSums(cluster_counts[]) > 0,]
    
    # Print the number of genes being analyzed from the pathway
    print(paste("Analyzing", nrow(cluster_counts), "genes from",
                all_paths_list[[list]][1,1]))
    ...
    print(paste("Running DEG using DESeq2 on genes in",
                all_paths_list[[list]][1,1]))
    ...
    for (effect in names) {
      ...
      hist_plot <- ggplot(results, aes(x = pvalue)) + 
                   ...
                   xlim(0, 1.05)
      print(hist_plot)
      ggsave(filename = paste0(hist_path, "DEG_pvalue_hist_", cluster, "_",
                               all_paths_list[[list]][1,1], "_", effect,
      ...
      ggsave(filename = paste0(vol_path, "vol_plot_", cluster, "_",
                               all_paths_list[[list]][1,1], "_", effect,
                               "_res.", chose_resolution, suffix, ".svg"),
                 height = 7, width = 7, plot = vol_plot)
      
      # Create object to hold all significant genes
      up_in_paths <- NULL
      
      if (dim(sig_results)[1] > 0) {
        ...
        ggsave(filename = paste0(vln_path, cluster, "_",
                                 all_paths_list[[list]][1,1], "_vln_plot_",
        ...
        write.csv(sig_results,
                  paste0(bulk_path, cluster, "_", all_paths_list[[list]][1,1],
                         "_DEGs_", effect, "_res", chose_resolution, suffix,
                         ".csv"))
        ...
        heatmap <- pheatmap(fitted, scale = "row", color = heatmap_colors(200),
                            ...
                            cluster_cols = FALSE)
        ggsave(filename = paste0(heat_path, cluster, "_",
                                 all_paths_list[[list]][1,1],
                                 "_pseudo_DEG_heatmap_", effect, "_res",
                                 chose_resolution, "_", suffix, ".svg"),
               height = 7, width = 11, plot = heatmap)
        if (effect == names[1]) {
          up_in_paths <- rbind(up_in_paths, sig_results)
        }
      }
    }
  }
  if (!is.null(up_in_paths)) {
    # Write the significant DEGs to a csv file.
    write.csv(up_in_paths,
              paste0(bulk_path, cluster, "_DEGs_in_all_pathways_", effect,
