...
bulk_path <- change_path(paste0(outpath, "/DEG/pseudobulk_by_group_batch/"))
full_path <- change_path(paste0(outpath, "/DEG/pseudobulk_by_group_batch/no_cutoff/"))
...
for (cluster in clusters) {
  ...
  bulk_path <- paste0(outpath, "/DEG/pseudobulk_by_group_batch/")
  full_path <- paste0(outpath, "/DEG/pseudobulk_by_group_batch/no_cutoff/")
  ...
  for (effect in names) {
    ...
    if (identical(method, "lfc") & dim(filter(results, padj < 0.1))[1] > 0) {
      ...
      for (pathway in pathway_list) {
        # Change output folders
        bulk_path <- change_path(paste0(outpath, "/DEG/pseudobulk_by_group_batch/no_cutoff/Wald/"))
        plots_path <- change_path(paste0(bulk_path, "plots/"))
        ...
        if (dim(gsea_results@result)[1] > 0) {
          bulk_path <- change_path(paste0(outpath,
                                          "/DEG/pseudobulk_by_group_batch/Wald/"))
          plots_path <- change_path(paste0(bulk_path, "plots/"))
