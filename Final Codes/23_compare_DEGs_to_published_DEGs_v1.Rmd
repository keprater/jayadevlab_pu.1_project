---
title: "23_compare_DEGs_to_published_DEGs_v1"
...
csv_path <- paste0("../output/", csv_path)
# Set output path
compare_path <- change_path(paste0(csv_path, "compare_to_published/"))
...
for (csv in 1:length(csvs)) {
  for (other in 1:ncol(ref)) {
    ...
    if (!is.null(list1$avg_logFC)) {
      ...
      write.csv(overlap_genes, paste0(compare_path, per, "_percent_down_",
    ...
    # Write the genes to a csv file.
    write.csv(overlap_genes, paste0(compare_path, per, "_percent_up_",
                                    colnames(ref)[other], "_vs_", csvs[csv],
                                    suffix, ".csv"), row.names = FALSE)
  ...
  if (!is.null(deg_clust)) {
    ...
    write.csv(get(paste0("genelist", csv)), paste0(csv_path, csvs[csv]))
  }
}
if (!is.null(deg_clust)) {
  write.csv(new_csv, csv_file)
}
```