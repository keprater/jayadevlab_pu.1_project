---
title: "10_snRNAseq_cluster_identity_detection_v2"
...
start_data <- "QCd_louvain_multi_clustered_15pcs_ws_20210331.rdata"
...
change_output_path <- "../output/output_subsetSCT_20210331/"

# Change suffix if needed, otherwise leave NULL
change_suffix <- "_subsetSCT_20210331"
...
if (!is.null(change_output_path)) {
  ...
  umaps_path <- file.path(outpath, "umaps/leiden_clust/15pcs/", fsep = "/")
  if (file.exists(umaps_path)) {
    cat("umaps directory path works!")
  } else {
    cat("umaps directory does not exist - creating")
    dir.create(file.path(outpath, "umaps", fsep = "/"))
    dir.create(file.path(outpath, "umaps/leiden_clust/", fsep = "/"))
...
plot_features <- function(genes, umap_name) {
  plot1 <- FeaturePlot(ss_data_norm, 
                       ...
                       min.cutoff = 0)
  print(plot1)
  ggsave(filename = paste0(umaps_path, cell_type, umap_name,
                           chose_resolution, suffix, ".png"), height = 7, 
         width = 11, plot = plot1)
}

check_genes <- function(pattern){
  str_subset(rownames(ss_data_norm@assays$integrated@scale.data),
             paste0("^",pattern,"$"))
}
...
plot_dot_plot <- function(genes, plot_name) {
  # change assay to ensure we can see all genes
  ...
  print(plot2)
  ggsave(filename = paste0(umaps_path, cell_type, plot_name,
...
if(identical(tissue_type, "brain")) {
  ...
  feature_genes <- c("SPI1", "C3", "CD74", "C1QB", "CCR2", "CX3CR1","GFAP", 
                     "S100B", "ALDH1L1", "AQP4", "MAP2", "SATB2", "CALB2", 
                     "RBFOX3", "SLC17A7", "GRIA1", "GRIN1", "GRIN2B", "GAD1", 
                     "GAD2", "SLC6A1", "SLC6A11","NES", "SOX2", "DCX", "NEUROD1",
                     "PDGFRA", "COL9A1", "COL20A1", "SOX8", "MYT1", "TMEM63A",
                     "ST18", "TGFBR2", "LEF1", "ITIH5")
  ...
  if(identical(cell_type, "pu1")) {
    ...
    ggsave(filename = paste0(umaps_path, cell_type,
                             "_general_gene_expression_cluster_heatmap_",
                             chose_resolution, suffix, ".pdf"),
           height = 7, width = 11, plot = plot3)
  }else{
    #Plot progenitor markers
    plot_features(c("NES", "SOX2", "DCX", "NEUROD1", "CALB2", "RBFOX3"),
                  "_progenitor_gene_expression_umap_")
    
    #Explore known neuronal markers
    plot_features(c("MAP2", "SATB2", "CALB2", "RBFOX3"),
    ...
    plot_features(c("SLC17A7", "GRIA1", "GRIN1", "GRIN2B"),
                  "_glut_gene_expression_umap_")
    
    #Plot gabaergic neuron markers
    plot_features(c("GAD1", "GAD2", "SLC6A1", "SLC6A11"),
