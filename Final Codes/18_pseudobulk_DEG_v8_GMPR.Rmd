...
meta_vars <- c("Study_Designation", "Sex", "Age", "APOE4_Allele")
...
interactions <- c("Study_Designation:APOE4_Allele", "Study_Designation:Age")
...
full_path <- change_path(paste0(outpath, "/DEG/pseudobulk_by_group/no_cutoff/"))
...
# Kenny disabled below, because I think the lowly expressed genes are still relevant for assessing overall "pseudobulk" library size
...
aggregate.voom = lapply(aggregate,function(x) {
  # Quantile normalize using limma::voom
  v = limma::voom(x,normalize.method='quantile')
  # Extract normalized expression matrix
  out = v$E
  # Order by not AD, AD
  out[,order(colnames(out) %in% subset(metadata_df,Study_Designation=='AD')$Pt_ID)]
})

# Normalize with GMPR
aggregate.gmpr = lapply(aggregate,function(x) {
  # Calculate size factor using GMPR
  size.factor = suppressWarnings(GMPR(t(as.matrix(x)),min_ct=1,intersect_no=1))
  # Extract normalized expression matrix
  out = t(t(x) / size.factor)
  # Order by not AD, AD
  out[,order(colnames(out) %in% subset(metadata_df,Study_Designation=='AD')$Pt_ID)]
})

for (i in 1:length(aggregate.voom)) {
  message(paste0(plots_path,'voom_cluster',i,'.pdf'))
  pdf(paste0(plots_path,'voom_cluster',i,'.pdf'))
    heatmap(aggregate.voom[[i]],Colv=NA,labRow=FALSE,main=paste('voom: cluster',names(aggregate.voom)[i]))
  dev.off()

  message(paste0(plots_path,'gmpr_cluster',i,'.pdf'))
  pdf(paste0(plots_path,'gmpr_cluster',i,'.pdf'))
    heatmap(aggregate.gmpr[[i]],Colv=NA,labRow=FALSE,main=paste('GMPR: cluster',names(aggregate.gmpr)[i]))
  dev.off()
}
```
...
for (cluster in clusters) {
  ...
  full_path <- paste0(outpath, "/DEG/pseudobulk_by_group/no_cutoff/")
  ...
  for (effect in names) {
    ...
    if (identical(method, "lfc") & dim(filter(results, padj < 0.1))[1] > 0) {
      ...
      full_path <- 
        change_path(paste0(outpath, "/DEG/pseudobulk_by_group/no_cutoff/lfc_GSEA"))
      plots_path <- change_path(paste0(bulk_path, "plots/"))
      ...
      full_path <- change_path(paste0(outpath,
                     "/DEG/pseudobulk_by_group/no_cutoff/log10p_GSEA"))
      plots_path <- change_path(paste0(bulk_path, "plots/"))
      ...
      for (pathway in pathway_list) {
        # Change output folders
        bulk_path <- change_path(paste0(outpath, "/DEG/pseudobulk_by_group/no_cutoff/Wald/"))
        plots_path <- change_path(paste0(bulk_path, "plots/"))
